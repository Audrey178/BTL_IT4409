        ...
        >
          ...
          <ScrollArea className="flex-1 px-8 pb-8">
            <div className="space-y-8">
              ...
              {/* AR MASKS */}
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant/60 mb-4">AR Masks</p>
                <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide pt-3">
                  {MASK_OPTIONS.map(mask => (
                     <button key={mask.id} onClick={() => toggleMask(mask.id)} className="shrink-0 w-16 flex flex-col items-center gap-2 group">
                        ...
                        <span className={`text-[10px] font-bold uppercase tracking-widest ${activeMasks.includes(mask.id) ? "text-primary" : "text-on-surface-variant/60"}`}>
                           {mask.label}
                        </span>
                     </button>
                  ))}
...
function ColorFilter({ color, label, active = false, onClick }: any) {
  return (
    <button onClick={onClick} className="shrink-0 w-16 flex flex-col items-center gap-2 group">
      ...
      />
      <span className={`text-[10px] font-bold uppercase tracking-widest ${active ? "text-primary" : "text-on-surface-variant/60"}`}>
        {label}
      </span>
    </button>
  );
